...
// Customized for use with EZT-MediaPlayer
...
(function WebNowPlaying() {
    ...
    function updateStorage(data) {
		...
		storage.is_paused = data.is_paused;
		storage.is_buffering = data.is_buffering;
    ...
    function updateInfo() {
        if (!Spicetify.Player.data && currState !== 0) {
            //ws.send("STATE:" + 0);
			ws.send(JSON.stringify({ message: $("STATE:" + 0).val(), path: '/recieve' }));
            currState = 0;
            return;
        }
		...
		ws.send(JSON.stringify({ message: JSON.stringify(storage), path: '/recieve' }));
