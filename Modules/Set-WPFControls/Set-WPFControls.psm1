...
function Set-WPFButtons
{
  ...
  try{
    ...
    #TODO: Need to finish - add UI button or menu option for miniplayer
    #----------------------------------------------
...
function Set-VideoPlayer
{
  ...
  try{
    switch ($Action) {
      'Open' {
        ...
        if($synchash.videoView){
          if(!$synchash.MiniPlayer_Viewer.isVisible -and $synchash.Window.isVisible -and $synchash.VideoView.Visibility -in 'Hidden','Collapsed' -and (!$synchash.YoutubeWebView2.CoreWebView2.IsDocumentPlayingAudio) -and $synchash.WebPlayer_State -eq 0 -and !$synchash.Youtube_WebPlayer_title -and ($synchash.VideoButton_ToggleButton.isChecked -or $synchash.MediaViewAnchorable.isFloating)){
            write-ezlogs ">>>> Video view is hidden and Main window is not hidden, MiniPlayer_Viewer not visible, Youtube webplayer not playing, unhiding video view" -Warning -loglevel 0 -Verboselog:$Verboselog
        ...
        write-ezlogs ">>>> Opening Video Player - Original Window.Top: $($synchash.Window.Top) - RestoreBounds: $($synchash.Window.RestoreBounds.Top) - Working area: $($PrimaryMonitor.WorkingArea.Height) - final: $OgPositionDiff"
      ...
      'Close' {
        write-ezlogs ">>>> Closing Video Player - Current Window.Top: $($synchash.Window.Top) - RestoreBounds: $($synchash.Window.RestoreBounds.Top)"
        if($synchash.VideoButton_ToggleButton.isChecked){
        ...
        if(!$synchash.MediaViewAnchorable.isfloating -and $synchash.VideoViewAirControl.Visibility -eq 'Visible'){
          write-ezlogs "| Collapsing VideoViewAirControl while closing" -loglevel 0 -Verboselog:$Verboselog
        ...
        if(!$synchash.VideoViewFloat.isVisible -and !$synchash.MediaViewAnchorable.isfloating){
          write-ezlogs "| Video Player is not floating, collapsing video view" -loglevel 0 -Verboselog:$Verboselog
      ...
      'FullScreen' {
        write-ezlogs ">>>> Setting Video Player to fullscreen" -loglevel 0 -Verboselog:$Verboselog
        if($synchash.Window.isVisible){
          $synchash.MediaViewAnchorable.FloatingLeft = $synchash.Window.Left
        }
        write-ezlogs ">>>> Undocking and Maximizing Video Player window"
        ...
        if($synchash.VideoViewFloat.WindowState -and $synchash.VideoViewFloat.WindowState -ne [System.Windows.WindowState]::Maximized){
          write-ezlogs "| Setting ViewViewFloat window state to Maximized" -loglevel 0 -Verboselog:$Verboselog
      ...
      'Normal' {
        write-ezlogs ">>>> Setting Video Player window state to normal" -loglevel 0 -Verboselog:$Verboselog
        ...
        if($synchash.Window.Top -ge 0){
          write-ezlogs "| Setting Video Player window top to main window top: $($synchash.Window.Top)" -loglevel 0 -Verboselog:$Verboselog
          $synchash.VideoViewFloat.Top = $synchash.Window.Top
        }else{
          $synchash.VideoViewFloat.Top = 0
          write-ezlogs "| Setting Video Player window top to 0" -loglevel 0 -Verboselog:$Verboselog
        }
      }
      'Maximized' {
        write-ezlogs ">>>> Maximizing Video Player Window" -loglevel 0 -Verboselog:$Verboselog
        $synchash.MediaViewAnchorable.IsMaximized = $true
        $synchash.VideoView_isFullScreen = $true
...
function Open-MiniPlayer
{
  ...
  try{
    ...
    if(!$synchash.MiniPlayer_Viewer.isVisible){
      write-ezlogs ">>>> Opening MiniPlayer view: Main Window.IsLoaded: $($synchash.Window.IsLoaded)"
    ...
    $Binding = [System.Windows.Data.Binding]::new()
    $Binding.Source = $synchash.Window
    $Binding.Path = "Title"
    $Binding.Mode = [System.Windows.Data.BindingMode]::OneWay
    [void][System.Windows.Data.BindingOperations]::SetBinding($synchash.MiniPlayer_Viewer,[MahApps.Metro.Controls.MetroWindow]::TitleProperty, $Binding)
    $Binding = [System.Windows.Data.Binding]::new()
    $Binding.Source = $synchash.Window
    $Binding.Path = "TaskbarItemInfo"
    $Binding.Mode = [System.Windows.Data.BindingMode]::OneWay
    [void][System.Windows.Data.BindingOperations]::SetBinding($synchash.MiniPlayer_Viewer,[MahApps.Metro.Controls.MetroWindow]::TaskbarItemInfoProperty, $Binding)
    ...
    if(!$Synchash.MiniPlayer_LoadedScriptblock -and $thisApp.Config.Dev_mode){
      $Synchash.MiniPlayer_LoadedScriptblock = {
        Param($Sender)
        try{
          write-ezlogs ">>>> Miniplayer has loaded - topmost: $($thisApp.Config.Mini_Always_On_Top)" -Dev_mode
        }catch{
    ...
    if(!$Synchash.MiniPlayer_ClosedScriptblock){
      $Synchash.MiniPlayer_ClosedScriptblock = {
        ...
        try{
          ...
          if($synchash.VideoViewAirControl.Visibility -in 'Hidden','Collapsed'){
            write-ezlogs "| Unhiding VideoViewAirControl" -Dev_mode
            $synchash.VideoViewAirControl.Visibility = 'Visible'
          }
        }catch{
    ...
    if(!$synchash.MediaViewAnchorable.isfloating -and $synchash.VideoView.Visibility -eq 'Visible'){
      write-ezlogs "Video Player is not floating, collapsing Video View while Miniplayer visible" -warning -LogLevel 0 -Verboselog:$Verboselog
      ...
      if($synchash.chat_WebView2.isVisible -or $synchash.Comments_Grid.Visibility -ne 'Collapsed'){
        write-ezlogs "| Hiding chat view" -warning -LogLevel 0 -Verboselog:$Verboselog
          ...
          try{
            ...
            if($Process -and $object.ProcessObject){
              ...
              if(-not [string]::IsNullOrEmpty($object.Now_Playing_Title)){
                write-ezlogs "[UPDATE-MAINPLAYER] >>>> Setting Now Playing Title Label: $($object.Now_Playing_Title)" -loglevel 0 -Verboselog:$object.verboselog
              ...
              if(-not [string]::IsNullOrEmpty($object.Now_Playing_Artist)){
                write-ezlogs "[UPDATE-MAINPLAYER] >>>> Setting Now Playing Artist Label: $($object.Now_Playing_Artist)" -loglevel 0 -Verboselog:$object.verboselog
              ...
              if($object.Add_VideoView -and $synchash.VideoView){
                if($synchash.VLC_Grid.children.name -notcontains 'VideoView'){
                  write-ezlogs "[UPDATE-MAINPLAYER] Adding VideoView to Vlc_Grid" -logtype Libvlc -loglevel 0 -Verboselog:$object.verboselog
              ...
              if($object.Remove_VideoView -and $synchash.VideoView){
                if($synchash.VLC_Grid.children.name -contains 'VideoView'){
                  write-ezlogs "[UPDATE-MAINPLAYER] Removing VideoView from Vlc_Grid" -logtype Libvlc -loglevel 0 -Verboselog:$object.verboselog
              ...
              if($object.New_MediaPlayer -and $synchash.libvlc){
                if($synchash.vlc.isPlaying){
                  write-ezlogs "[UPDATE-MAINPLAYER-New_MediaPlayer] | Stopping existing vlc session" -warning -logtype Libvlc
                  $synchash.VLC_IsPlaying_State = $false
                  $synchash.vlc.stop()
                }
                Add-VLCRegisteredEvents -synchash $synchash -thisApp $thisApp -UnregisterOnly
                if($synchash.vlc.media -is [System.IDisposable]){
                  write-ezlogs "[UPDATE-MAINPLAYER-New_MediaPlayer] | Disposing existing vlc.media" -logtype Libvlc
                  $synchash.vlc.media.dispose()
                  $synchash.vlc.media = $Null
                }
                if($synchash.VideoView.MediaPlayer.media -is [System.IDisposable]){
                  write-ezlogs "[UPDATE-MAINPLAYER-New_MediaPlayer] | Disposing existing VideoView.MediaPlayer.media" -logtype Libvlc
                  $synchash.VideoView.MediaPlayer.media.dispose()
                  $synchash.VideoView.MediaPlayer.media = $Null
                }
                if($synchash.vlc -is [System.IDisposable]){
                  write-ezlogs "[UPDATE-MAINPLAYER-New_MediaPlayer] | Disposing existing vlc session" -warning -logtype Libvlc
                  $synchash.vlc.dispose()
                  $synchash.vlc = $Null
                  $synchash.VideoView.MediaPlayer = $Null
                }
                if(!$synchash.vlc){
                  write-ezlogs "[UPDATE-MAINPLAYER-New_MediaPlayer] | Creating new vlc mediaplayer session" -warning -logtype Libvlc
                  ...
                  write-ezlogs "[UPDATE-MAINPLAYER-New_MediaPlayer] | Using existing vlc mediaplayer session" -logtype Libvlc
                }
                if($thisapp.config.Current_Audio_Output -and $synchash.vlc.AudioOutputDeviceEnum){
                  ...
                  if($device.Description -ne 'Default'){
                    write-ezlogs "[UPDATE-MAINPLAYER-New_MediaPlayer] >>>> Setting Audio Output device: $($device.Description)" -showtime -logtype Libvlc
                ...
                if($object.memory_stream){
                  ...
                  if($object.media_link){
                    if([System.IO.File]::Exists($object.media_link)){
                      write-ezlogs "[UPDATE-MAINPLAYER] | Medialink is local path link: $($object.media_link)" -logtype Libvlc -LogLevel 0 -Verboselog:$object.VerboseLog
                      if($thisApp.Config.Libvlc_Version -eq '4'){
                      ...
                      write-ezlogs "[UPDATE-MAINPLAYER] | Medialink is URL link: $($object.media_link)" -logtype Libvlc -LogLevel 0 -Verboselog:$object.VerboseLog
                      if($thisApp.Config.Libvlc_Version -eq '4'){
                        $ParseOption = [LibVLCSharp.MediaParseOptions]::ParseNetwork
                    ...
                    if($object.vlcurl){
                      write-ezlogs "[UPDATE-MAINPLAYER] | vlcurl is URL link: $($object.vlcurl)" -logtype Libvlc -LogLevel 0 -Verboselog:$object.VerboseLog
                      if($thisApp.Config.Libvlc_Version -eq '4'){
                        $ParseOption = [LibVLCSharp.MediaParseOptions]::ParseNetwork
                        $from_path = [LibVLCSharp.FromType]::FromLocation
                      }else{
                      ...
                      write-ezlogs "[UPDATE-MAINPLAYER] | vlcurl is local path link: $($object.vlcurl)" -logtype Libvlc -LogLevel 0 -Verboselog:$object.VerboseLog
                ...
                if(!$thisApp.Config.Use_HardwareAcceleration){
                  write-ezlogs "[UPDATE-MAINPLAYER] | Disabling Hardware Acceleration" -logtype Libvlc
                ...
                if($thisApp.Config.Enable_EQ -and $([string]$synchash.VLC.Media.Mrl).StartsWith("dshow://")){
                  write-ezlogs "[UPDATE-MAINPLAYER] | Enabling dshow capture for virtual audio cable" -Warning -logtype Libvlc
                  ...
                  try{
                    write-ezlogs "[UPDATE-MAINPLAYER] | Set FileCaching to 1000" -logtype Libvlc
                    ...
                    if(([uri]$object.vlcurl).LocalPath.StartsWith("\\") -or [system.io.driveinfo]::new(([uri]$object.vlcurl).LocalPath).DriveType -eq 'Network'){
                      if($thisapp.config.vlc_Arguments -notmatch 'network-caching='){
                        ...
                        write-ezlogs "[UPDATE-MAINPLAYER] | File appears to be located on a network drive/share. Not adjusting cache as a custom network-caching setting was found within vlc_Arguments: $($thisapp.config.vlc_Arguments)" -logtype Libvlc
                      }
                    }
                  }catch{
                  ...
                  write-ezlogs "[UPDATE-MAINPLAYER] | Media is a Live Stream" -logtype Libvlc
                  ...
                  write-ezlogs "[UPDATE-MAINPLAYER] | Set Network-caching to 1000" -logtype Libvlc
                  $synchash.VLC.Media.AddOption(":network-caching=1000")
                  $synchash.vlc.NetworkCaching = 1000
                }elseif((Test-URL $object.vlcurl)){
                  write-ezlogs "[UPDATE-MAINPLAYER] | Set Network-caching to 500" -logtype Libvlc
                ...
                if($thisapp.config.Enable_EQ -and $synchash.vlc -and !$synchash.EQ_Timer.isEnabled){
                  write-ezlogs "[UPDATE-MAINPLAYER] | Executing EQ_Timer" -logtype Libvlc -LogLevel 0 -Verboselog:$object.VerboseLog
                  $synchash.EQ_Timer.Start()
                }
                if($object.Start_media_Timer){
                  write-ezlogs "[UPDATE-MAINPLAYER] | Starting Media Timer" -LogLevel 0 -Verboselog:$object.VerboseLog
      ...
      $synchash.MainWindow_Update_Timer.add_tick({
          try{
            ...
            if($Process -and $object.ProcessObject){
              ...
              if($object.Hide){
                if($synchash.MiniPlayer_Viewer){
                  write-ezlogs ">>>> Miniplayer is open - hiding main window by setting opacity to 0" -LogLevel 0 -Verboselog:$object.VerboseLog
                  $synchash.window.ShowActivated = $false #Prevent window from activating/taking focus while rendering
                  $synchash.window.ShowInTaskbar = $false
                  $synchash.window.Opacity = 0
      ...
      $synchash.Reset_MediaPlayer_timer.add_tick({
          try{
            ...
            if($synchash.VLC_Grid.children -contains $synchash.Webview2){
              write-ezlogs "[Reset-MainPlayer] >>>> Removing Webview2 from Vlc_Grid" -LogLevel 0 -Verboselog:$this.tag.Verboselog
            ...
            if($synchash.Webview2_Grid.children -contains $synchash.YoutubeWebView2){
              write-ezlogs "[Reset-MainPlayer] | Removing Youtubewebview2 from Webview2_Grid" -LogLevel 0 -Verboselog:$this.tag.Verboselog
            ...
            if($synchash.VLC_Grid.children -contains $synchash.YoutubeWebView2){
              write-ezlogs "[Reset-MainPlayer] | Removing YoutubeWebView2 from Vlc_Grid" -LogLevel 0 -Verboselog:$this.tag.Verboselog
            ...
            ######
            if($synchash.VLC_Grid.children.name -notcontains 'VideoView'){
              Write-EZLogs '[Reset-MainPlayer] | Adding VideoView to VLC_Grid' -LogLevel 0 -Verboselog:$this.tag.Verboselog
          ...
          try{
            ...
            if($synchash.MediaView_Image -and $Thumbnail){
              try{
                ...
                write-ezlogs "An exception occurred setting Media_Image to source: $Thumbnail" -CatchError $_
              }
