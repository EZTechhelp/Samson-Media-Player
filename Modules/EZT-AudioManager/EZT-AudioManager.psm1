...
function Set-AudioSessions
{
  ...
  try{
    ...
    $cscore_Scriptblock = {
      ...
      try{
        write-ezlogs "[Set-AudioSessions] #### Looking for New Orphaned Audio Sessions" -logtype Libvlc -linesbefore 1
        $default_output_Device = [CSCore.CoreAudioAPI.MMDeviceEnumerator]::DefaultAudioEndpoint([CSCore.CoreAudioAPI.DataFlow]::Render,[CSCore.CoreAudioAPI.Role]::Multimedia)
        ...
        $searcher.Dispose()
        if($default_output_Device){
          write-ezlogs "[Set-AudioSessions] | Found Default Audio Output Device: $($default_output_Device) - Current_Audio_Session_Group: $($synchash.Current_Audio_Session.GroupingParam)" -logtype Libvlc
          $synchash.AudiosessionManager = [CSCore.CoreAudioAPI.AudioSessionManager2]::FromMMDevice($default_output_Device)
          $synchash.AudioSessions = $synchash.AudiosessionManager.GetSessionEnumerator()
            ...
            try{
              ...
              if($audionsessioncontrol.GroupingParam -eq $synchash.Current_Audio_Session.GroupingParam -or $audionsessioncontrol.ProcessID -eq $synchash.Current_Audio_Session.ProcessID){
                if($audionsessioncontrol.DisplayName -ne "$($thisApp.Config.App_Name) Media Player - $($thisApp.Config.App_version)" -and $audionsessioncontrol.DisplayName -notmatch "WebPlayer EQ"){
                  write-ezlogs "[Set-AudioSessions] | Updating Default Audio Output Device session DisplayName from ($($audionsessioncontrol.DisplayName)) to ($($thisApp.Config.App_Name) Media Player - $($thisApp.Config.App_version)) - GroupParam: $($synchash.Current_Audio_Session.GroupingParam)" -logtype Libvlc -LogLevel 0 -Verboselog:$Verboselog
                  $audionsessioncontrol.DisplayName = "$($thisApp.Config.App_Name) Media Player - $($thisApp.Config.App_version)"
                }
                if($audionsessioncontrol.IconPath -ne "$($thisApp.Config.Current_Folder)\Resources\Samson_Icon_NoText1.ico"){
                  write-ezlogs "[Set-AudioSessions] | Updating Default Audio Output Device session IconPath from ($($audionsessioncontrol.IconPath)) to ($($thisApp.Config.Current_Folder)\Resources\Samson_Icon_NoText1.ico) - GroupParam: $($synchash.Current_Audio_Session.GroupingParam)" -logtype Libvlc -LogLevel 0 -Verboselog:$Verboselog
                  $audionsessioncontrol.IconPath = "$($thisApp.Config.Current_Folder)\Resources\Samson_Icon_NoText1.ico"
                }
                if($synchash.Current_Audio_Session.GroupingParam -and $audionsessioncontrol.GroupingParam -ne $synchash.Current_Audio_Session.GroupingParam){
                  write-ezlogs "[Set-AudioSessions] | Adding audio session (Group: $($audionsessioncontrol.GroupingParam)) to main GroupParam: $($synchash.Current_Audio_Session.GroupingParam)" -logtype Libvlc -LogLevel 0 -Verboselog:$Verboselog
                ...
                if($audionsessioncontrol.IsSingleProcessSession){
                  ...
                  write-ezlogs "[Set-AudioSessions] | Audio session (ProcessID: $($audionsessioncontrol.ProcessID)) is IsSingleProcessSession, ParentProcessId: $ParentProcessId" -logtype Libvlc -LogLevel 0 -Verboselog:$Verboselog
                  $webplayer_process = $webviewProcesses | Where-Object {$_.ParentProcessId -eq $PID}
                  if($webplayer_process){
                    write-ezlogs "[Set-AudioSessions] | Audio session is from a Web Player -- ProcessID: $($webplayer_process.ProcessId)" -logtype Libvlc
                    if($synchash.Managed_AudioSession_Processes -notcontains $webplayer_process.ProcessId){
                      write-ezlogs "| Registering Audio Session for WebPlayer process id: $($webplayer_process.ProcessId)" -logtype Libvlc
                      $null = $synchash.Managed_AudioSession_Processes.add($webplayer_process.ProcessId)
                    }
                  }
                }
                #TODO: Yikes clean up this godawful mess
