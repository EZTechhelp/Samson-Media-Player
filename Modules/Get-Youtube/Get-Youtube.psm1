        ...
        if($youtube_id){
          try{
            if($youtube_type -eq 'YoutubePlaylist'){
              write-ezlogs "Getting info for playlist $playlist - $youtube_id" -showtime -logtype Youtube -loglevel 0 -Verboselog:$Verboselog
    ...
    $synchash.Videos_toProcess | & { process {
        ...
        lock-object -InputObject $synchash.All_Youtube_Media.SyncRoot -ScriptBlock {
          if($synchash.processed_Youtube_tracks -notcontains $encodedid){
            write-ezlogs ">>>> Adding Youtube Video to All_Youtube_Media: URL: $($url) - ID: $($id) - Title: $($title)" -logtype Youtube -loglevel 0 -Verboselog:$Verboselog
      ...
      if($target_PlaylistID -and $Media.id){
        ...
        if($add.contentDetails.videoId){
          ...
          if($thisApp.Config.Youtube_Playlists -notcontains $target_Url){
            try{
              write-ezlogs "| Adding new Youtube Playlist URL to config: $($target_Url) - Playlist Name: $($targetplaylist_Name)" -showtime -logtype Youtube -loglevel 0 -Verboselog:$Verboselog
              ...
              if($Playlist_Profile -and $target_Url){
                ...
                write-ezlogs "| Saving new Youtube Playlist profile to $Playlist_Profile_path" -showtime -logtype Youtube -loglevel 0 -Verboselog:$Verboselog
      ...
      foreach($media in $media){
        ...
        if($Media.type -match 'YoutubePlaylistItem' -or $Media.url -match 'youtube' -or $Media.Source -eq 'Youtube'){
          ...
          if([string]::IsNullOrEmpty($target_playlist.playlist_id)){
            ...
            $target_playlist | ForEach-Object {
      ...
      $checkYoutube_scriptblock = {
        ...
        try{
          ...
          if($internet_Connectivity){
            ...
            $synchash.newytplaylists = 0
            $newvideos = 0
            $synchash.removedvideos = 0
            $synchash.newytchannels = 0
                ...
                try{
                  ...
                  if($ytsubs.snippet.resourceId.channelId){
                    foreach($sub in $ytsubs){
                      ...
                      if($YoutubePlaylists_itemsArray.path -notcontains $playlisturl){
                        write-ezlogs "Adding Youtube Subscription Channel URL $playlisturl" -showtime -logtype Youtube -loglevel 0 -Verboselog:$Verboselog
                        if(!$YoutubePlaylists_itemsArray.Number){
                          $Number = 1
                        }else{
                          $Number = $YoutubePlaylists_itemsArray.Number | Select-Object -last 1
            ...
            if($synchash.newytplaylists -le 0 -and $synchash.newytchannels -le 0 -and $playlists_toRemove.count -le 0 -and $newvideos -le 0 -and $synchash.removedvideos -le 0){
              write-ezlogs "No changes to Youtube playlists were found" -showtime -warning -logtype Youtube
              return
            }else{
              write-ezlogs "Found $($synchash.newytplaylists) playlists, $($synchash.newytchannels) new channels, $($newvideos) new playlists videos, $($synchash.removedvideos) removed playlist videos -- found $($playlists_toRemove.count) playlists to remove" -showtime -logtype Youtube
          ...
          $synchash.newytplaylists = $Null
          $synchash.removedvideos = $Null
          $synchash.newytchannels = $Null
        }
      }
      if($Use_runspace){
        #$Variable_list = Get-Variable -Scope Local | & { process {if ($_.Options -notmatch "ReadOnly|Constant"){$_}}}
        Start-Runspace $checkYoutube_scriptblock -arguments $PSBoundParameters -StartRunspaceJobHandler -synchash $synchash -logfile $thisApp.Config.Log_file -runspace_name "checkYoutube_runspace" -thisApp $thisApp
        $Variable_list = $Null
      }else{
