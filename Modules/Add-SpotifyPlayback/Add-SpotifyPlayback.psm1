...
function Add-SpotifyPlayback
{
  ...
  if($SpotifyType -and $LinkUri){
    write-ezlogs ">>>> Adding Spotify media $LinkUri - Type: $SpotifyType" -showtime -logtype Spotify
    if($LinkUri -match '\?'){
    ...
    if($track){
      ...
      if($AddtoQueue){
        ...
        write-ezlogs "| Adding Spotify track '$($media.title) - $($media.Artist)' to playlist: $AddtoPlaylist"
