  ...
  if ([bool]([System.Uri]$Source).IsUnc -or $Source -like 'http?://*') {
    ...
    if (!(Test-Path -Path $NewFilePath) -or $ForceRefresh) {
      if ([bool]([System.Uri]$Source).IsUnc) {
        ...
        $null = Invoke-RestMethod -Uri $Source -OutFile $NewFilePath
      }
    }
...
function Get-BTHistory {
  ...
  if (!(Test-Path -Path "Registry::\HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Notifications\Settings\$AppId")) {
...
function New-BTAppId {
  ...
  $RegPath = 'Registry::\HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Notifications\Settings'
...
function Remove-BTNotification {
  ...
  } elseif (!(Test-Path -Path "Registry::\HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Notifications\Settings\$AppId")) {
...
$searcher.Dispose()
$WinMajorVersion = $results.version.Split('.')[0]
$results.Dispose()
