  ...
  try {
    ...
    if($Application.Name){
      ...
      if($secretstore){
        ...
        if ($Token.expires) {
          Set-Secret -Name Spotyexpires -Secret $Token.expires -Vault $Name
        }
        if ($Token.access_token) {
          Set-Secret -Name Spotyaccess_token -Secret $Token.access_token -Vault $Name
        }
        if ($Token.scope) {
          Set-Secret -Name Spotyscope -Secret $Token.scope -Vault $Name
        }
        if ($Token.refresh_token) {
          Set-Secret -Name Spotyrefresh_token -Secret $Token.refresh_token -Vault $Name
        }        
        if ($Token.token_type) {
