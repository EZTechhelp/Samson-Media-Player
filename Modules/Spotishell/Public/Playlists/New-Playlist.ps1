...
function New-Playlist {
  ...
  if([string]::IsNullOrEmpty($UserId)){
    try{
      $profile = (Get-CurrentUserProfile -ApplicationName $thisApp.Config.App_Name)
    }catch{
      write-ezlogs "An exception occurred executing Get-CurrentUserProfile in Spotishell" -catcherror $_
      return
    }
  }
