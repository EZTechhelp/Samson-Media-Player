function Update-DSRichPresence {
  ...
  process {
    ...
    if($State){
      ...
      if($Chars -ge 128){
        $State = "$([string]$State.subString(0, [System.Math]::Min(124, $State.Length)).trim())..." 
        write-ezlogs "[Set-DiscordPresense] Provided state string is $($Chars) characters long (128 max allowed) - trimming to: $State" -warning -logtype Discord
      }
    }
    if($Details){
