<#
    ...
    .NOTES
    #TODO: Requires heavy refactor or more likely complete rewrite
  ...
  foreach ($a in $apps)
  {
    ...
    elseif($a_InstallLocation)
    {
      try
      {
        ...
        if($b_InstallFolder){
          $b_InstallDate = Get-ItemProperty $b_InstallFolder | Select-Object CreationTime
          if($b_InstallDate){
    ...
    elseif($a_InstallLocation)
    {
      try
      {
        ...
        if([system.io.directory]::Exists($a_InstallLocation)){
    ...
    $all_apps_output = [System.Collections.Generic.List[Object]]::new()     
