...
function Get-TwitchStatus
{
  ...
  if($CheckAll){    
    if($thisApp.Config.Verbose_logging){write-ezlogs ">>>> Getting Status of all known Twitch Streams" -showtime -color cyan -logfile:$log}    
      ...
      if(-not [string]::IsNullOrEmpty($TwitchStreams) -and @($TwitchStreams).count -gt 0){       
        $checktwitch_scriptblock = {       
          if([System.IO.File]::Exists($AllYoutube_Media_Profile_File_Path)){
            if($thisApp.Config.Verbose_logging){write-ezlogs " | Importing Youtube Media Profile: $AllYoutube_Media_Profile_File_Path" -logfile:$log -showtime -enablelogs}
              ...
              try{
                ...
                if(!$TwitchAPI.type){
                  ...
                  if($Media.Live_Status -ne 'Offline' -or $media.Status_Msg -ne ''){
                    write-ezlogs ">>>> [Get-TwitchStatus] Updating $($Media.title) with status Offline from $($Media.Live_Status)" -showtime -logfile:$log
                  ...
                  if($twitch_status -ne $Media.Live_Status -or ("- $($TwitchAPI.game_name)" -ne $media.Status_msg)){
                    ...
                    write-ezlogs ">>>> [Get-TwitchStatus] Updating $($Media.title) with status $($twitch_status) from $($Media.Live_Status)" -showtime -logfile:$log
            ...
            if($changes -gt 0){
              try{
                write-ezlogs ">>>> [Get-TwitchStatus] Updating $($changes) changes for Twitch streams" -showtime -logfile:$log
              ...
              write-ezlogs "[Get-TwitchStatus] No changes were found for any Twitch Streams" -showtime -logfile:$log
            }                                                       
          }                                                      
        }.GetNewClosure()
